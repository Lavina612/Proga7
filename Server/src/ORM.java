...
public class ORM {
    ...
    public ORM(PooledConnection pooledConnection) {
        ...
        for(Field field: fields) {
            ...
            if(column!=null) {
                ...
                if (dateTime != null) {
                    ...
                    values.append("'").append(time.toString()).append("',");
                } else
    ...
    public void updateObject(Object obj)throws ORMException, ClassNotFoundException, IllegalAccessException, SQLException{
        ...
        StringBuilder queryObj = new StringBuilder("update ").append(table.name()).append(" set ");
        Collections.addAll(fields, cl.getDeclaredFields());
        Class<?> tClass = cl.getSuperclass();
        while(tClass!=null){
            Collections.addAll(fields, tClass.getDeclaredFields());
        ...
        for(Field field: fields){
            ...
            if (dateTime != null && column!=null) {
                ...
                queryObj.append(column.name() + "=").append("'").append(time.toString()).append("',");
            } else
            if(column!=null && idi==null){
                if(field.getType()==String.class)
                    queryObj.append(column.name()).append("='").append(field.get(obj)).append("',");
                else queryObj.append(column.name()).append("=").append(field.get(obj)).append(",");
            }
            field.setAccessible(false);
        }
        queryObj.deleteCharAt(queryObj.length()-1);
        queryObj.append(" where ").append(idName).append("=").append(id).append(";");
        ...
        connection.setAutoCommit(false);
        Statement statement = connection.createStatement();
        statement.execute(queryObj.toString());
        for(String quaryProp: queriesProp){
            statement.execute(quaryProp);
        }
        connection.commit();
    ...
    public <T> Vector<T> getAllObjects(Class<T> cl)throws ORMException, ClassNotFoundException, SQLException, IllegalAccessException,
        ...
        while (rowSet.next()) {
            ...
            for(String column: match.keySet()){
                ...
                if(date!=null){
                    ZonedDateTime time = ZonedDateTime.parse(rowSet.getString(column));
                    System.out.println(time);
                    Class<?> dateType = field.getType();
                //    Method fac = dateType.getMethod("ofInstant", Instant.class, ZoneId.class);
                    field.set(newInstance, time/*fac.invoke(null, rowSet.getTimestamp(column).toLocalDateTime().toInstant(ZoneOffset.UTC), ZoneOffset.UTC)*/);
                }else {
